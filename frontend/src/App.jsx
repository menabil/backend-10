...
function App() {
  ...
  // ১. নতুন Todo তৈরি করা (ফাইল সহ FormData ব্যবহার করে)
  let handelClick = async () => {
    ...
    if (image) {
      formData.append("image", image); // backend-এর upload.single("image") এর সাথে মিলিয়ে 'image' দিতে হবে
    }
  ...
  // এডিটের সময় স্টেটগুলোতে ডাটা বসানো
  let handelEdit = (item) => {
    setTask(item.task);
    setPriority(item.priority || "");
    setStatus(item.status || "");
  ...
  // ২. Todo আপডেট করা (নতুন ছবি থাকলে সেটাও সহ)
  let handelUpdate = async () => {
    const formData = new FormData();
    formData.append("task", task);
    formData.append("priority", priority);
  ...
  const resetForm = () => {
    ...
    if (fileInputRef.current) {
      fileInputRef.current.value = ""; // ফাইল ইনপুট ক্লিয়ার করা
    }
  };
            ...
            {data.map((item, index) => (
              <li key={item._id || item.id || index} className="flex flex-col sm:flex-row justify-between items-start sm:items-center py-3.5 first:pt-0 last:pb-0 gap-3">
                ...
                <div className="flex items-center gap-3 overflow-hidden max-w-full sm:max-w-[50%]">
                  {/* ইমেজ রেন্ডার করার কোড */}
                  {item.path ? (
                    <img
                      src={`http://localhost:5000${item.path}`}
                      alt="task"
                      className="w-10 h-10 object-cover rounded-lg border border-zinc-700 shrink-0 bg-zinc-800"
                      onError={(e) => { e.target.style.display = 'none'; }} // কোনো ভুল পাথে ইমেজ লোড না হলে থাম্বনেইল হাইড হয়ে যাবে
                    />
                  ) : (
                    // ইমেজ না থাকলে একটি ডিফল্ট আইকন দেখানো যেতে পারে
                  ...
                  <span className="font-medium text-zinc-200 break-words">
                    {item.task}
                  </span>
                </div>
